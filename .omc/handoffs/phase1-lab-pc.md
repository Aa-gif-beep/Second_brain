...
5. `obsidian-vault` 폴더 등록 (**폴더 ID를 반드시 `obsidian-vault`로 통일** — 다른 기기와 폴더 공유 수락할 때 ID가 같아야 매칭됨):
   ```
   syncthing.exe cli config folders add --id=obsidian-vault --label=ObsidianVault --path="<Sync\ObsidianVault 경로>" --type=sendreceive
   ```
6. `ObsidianVault/.stignore`에 UI 충돌 방지용 패턴 추가 (이미 vault 안에 있으니 노트북에서는 안 해도 됨, sync되면 같이 옴):
   ```
